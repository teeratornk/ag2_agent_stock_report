...
# Engineer revision – ensure 'Adj Close' column exists (2025-10-23)
...
def fetch_stock_data(ticker: str, start_date: str, end_date_exclusive: str) -> pd.DataFrame:
    ...
    try:
        ...
        # ------------------------------------------------------------------
        # Guarantee presence of 'Adj Close'
        # ------------------------------------------------------------------
        if "Adj Close" not in df.columns:
            if "Close" in df.columns:
                ...
                raise KeyError("Neither 'Adj Close' nor 'Close' price columns found in data.")
...
def set_plot_style() -> None:
    """Apply a white-grid style using Matplotlib’s built-in styles."""
    try:
